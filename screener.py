...
Pipeline:
  1. Load a US equity universe.
  2. Batch-download daily OHLCV, compute 1d change / volume / 60d avg vol / relative volume.
  3. Split into UP-on-larger-volume and DOWN-on-larger-volume buckets (your TradingView filters).
  4. For survivors, pull fundamentals and compute the 7-test composite score (your Excel logic).
  5. Keep score >= SCORE_MIN, take the most liquid TOP_N from each side.
  6. Pull annual + quarterly statements -> YoY/QoQ trends for the detail panel.
  7. Write a single self-contained dashboard.html (data embedded, no internet needed to view).

Run it daily (cron / Task Scheduler). Re-running overwrites dashboard.html.
